...
import { ValidateLoginForm } from "@/helpers/loginValitadion";
import { useRouter } from "next/navigation";
import { IUserDataLogin } from "@/interfaces/interfaces";
import Cookies from "js-cookie";

const Login: React.FC = () => {
  const initialState: IUserDataLogin = {
    email: "",
    password: "",
  };

  const router = useRouter();
  const [formData, setFormData] = useState<IUserDataLogin>(initialState);
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const { valid, errors: validationErrors } = ValidateLoginForm(formData);
